...
//        ADDRESS=getAddressText(this.lastLatLng);
        getAddressTextNew(this.lastLatLng);
        FusedLocation.location=location;
        Map map=new HashMap();
        map.put("mobile",Preferences.getInstance(context).getMobile());
        map.put("lastUpdate",Helper.getCurrentDate()+" "+Helper.getCurrentTime());
        map.put("lat",location.getLatitude());
        map.put("long",location.getLongitude());
//        ApiCallService.action2(context,map,ApiCallService.Action.ACTION_ADD_LIVE_LOCATION);
//        stopLocationUpdates();
    }
